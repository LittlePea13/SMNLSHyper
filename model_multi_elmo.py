...
class HyperModel(nn.Module):
    ...
    def forward(self, meta_pred, predicted, squezeed_lengths = torch.LongTensor(1).to(device=torch.device('cuda')), doc_lengths = torch.LongTensor(1).to(device=torch.device('cuda'))):
        ...
        #average elmo embeddings
        averaged_docs = self.averaging(predicted)
    ...
    def averaging(self, out_embedding):

        averaged_sentences = torch.div((out_embedding.sum(dim=1)), out_embedding.size(1), out=None)
        print("Size of averaged sen", averaged_sentences.size())
