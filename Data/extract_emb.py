...
def hyperpart_elmo(infile, outfile, maxsent):
    hyp_elmo = []
    hyp_label = []
    hyp_len = []
    with open(infile, "rt", encoding="utf8") as inp:
        ...
        for enum,line in enumerate(inp):
            ...
            sents.extend(tmp)
            sents = '\n'.join(sents)
            file = open(outfile, "a", encoding="utf8")
            file.write(sents)
            file.close()
    ...
    try:
        subprocess.check_call(['allennlp elmo Hyp_article.txt elmo_hyp.hdf5 --average'], shell=True)
    ...
    #call('allennlp elmo Hyp_article.txt elmo_hyp.hdf5 --average', shell=True)
    #! allennlp elmo Hyp_article.txt elmo_hyp.hdf5 --average  
...
if __name__ == "__main__":
    infile = 'output.tsv'
    hyperpart_elmo(infile, 'Hyp_article.txt', 100)